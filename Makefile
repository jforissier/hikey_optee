...
all: build-bl1 build-bl2 build-bl31 build-bl33 build-fip build-lloader build-ptable build-linux build-dtb build-boot-fat build-nvme

clean: clean-bl1-bl2-bl31-fip clean-bl33 clean-lloader-ptable clean-linux-dtb clean-boot-fat

cleaner: clean cleaner-nvme cleaner-aarch64-gcc

distclean: cleaner distclean-aarch64-gcc
...
help:
	...
	@echo "  fastboot flash boot $(BOOT-FAT)"
...
BOOT-FAT = boot-fat.uefi.img

ifneq ($(filter all build-linux,$(MAKECMDGOALS)),)
boot-fat-deps += build-linux
endif
ifneq ($(filter all build-dtb,$(MAKECMDGOALS)),)
boot-fat-deps += build-dtb
endif

.PHONY: build-boot-fat
build-boot-fat:: $(boot-fat-deps)
build-boot-fat $(BOOT-FAT):: $(LINUX) $(DTB)
	$(ECHO) '  GEN     $@'
	$(Q)sudo -p "[sudo] Password:" true
	$(Q)if [ -d .tmpbootfat ] ; then sudo rm -rf .tmpbootfat ; fi
	$(Q)mkdir -p .tmpbootfat
	$(Q)dd if=/dev/zero of=boot-fat.uefi.img bs=512 count=131072 status=none
	$(Q)sudo mkfs.fat -n "BOOT IMG" boot-fat.uefi.img >/dev/null
	$(Q)sudo mount -o loop,rw,sync boot-fat.uefi.img .tmpbootfat
	$(Q)sudo cp $(LINUX) $(DTB) .tmpbootfat
	$(Q)sudo $(SHELL) -c "touch initrd && echo initrd | cpio -o --quiet >.tmpbootfat/initrd.img && rm -f initrd"
	$(Q)sudo umount .tmpbootfat
	$(Q)sudo rm -rf .tmpbootfat

clean-boot-fat:
	$(ECHO) '  CLEAN   $@'
	$(Q)rm -f boot-fat.uefi.img
